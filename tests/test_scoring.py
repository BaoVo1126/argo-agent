"""Credibility: provenance, corroboration, and what the threshold refuses."""
...
def health_for(*urls: str) -> HealthRecord:
    """A record in which the given domains have already proved themselves.

    Most of these tests are about the arithmetic of scoring rather than about
    verification, so they start from "these have worked before" -- the state any
    machine reaches after one successful run.
    """
    record = HealthRecord()
    for url in urls:
...
def test_government_and_bank_are_read_off_the_url_not_the_name():
    """A source cannot claim a tier; the tier comes from the address fetched."""
...
def test_a_bank_needs_corroboration_to_pass():
    """40 + 10 for structure is 50: one point short on its own, over the line
    once something independent reports the same numbers."""
...
def test_an_aggregator_stays_out_even_when_it_is_right():
    """Agreeing with a bank is not the same as being accountable for a number."""
    scores = score([
...
def test_a_different_quantity_is_not_corroboration():
    """A central rate and a commercial rate are both real and 5% apart, which
    is not two readings of one thing."""
    scores = score([
        evidence("vcb", "https://www.vietcombank.com.vn/a", series(26000)),
...
def test_a_crowd_of_copies_cannot_outvote_provenance():
    """Corroboration is capped, so five aggregators cannot manufacture a pass."""
    ...
    assert all(s.total == 15 + 30 + 10 for s in scores)   # cap applied
...
def test_too_little_overlap_is_not_agreement():
    """Two series that meet on three days agree about nothing."""
    scores = score([
...
# --- verification: a tier has to be earned before it pays -----------------

def test_a_domain_never_scraped_scores_nothing_for_provenance():
    """The tier table used to pay 50 points to a central bank nobody had ever
    fetched a byte from. An impressive suffix is not evidence."""
...
def test_the_threshold_is_suspended_until_an_official_source_is_verified():
    """60 is the point where one authoritative publisher passes alone. With
    none verified, enforcing it would reject everything for a reason about this
    project's own coverage rather than about the data."""
    ...
    assert scores[0].accepted is True     # it returned rows, and that is the bar
    assert scores[0].total == 50          # the score is still reported honestly
...
def test_verification_survives_a_later_outage(tmp_path):
    """A bank being down for an afternoon is not evidence it was never a bank,
    so the record answers "has this ever been real?", not "is it up now?"."""
